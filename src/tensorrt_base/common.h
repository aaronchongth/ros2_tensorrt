/*
This header contains commonly used functions in the running of TensorRT engines,
this can be obtained from the TensorRT samples, with minor changes here and
there
*/
...
inline std::vector<std::string> classify(const vector<string>& refVector,
                                         ...
                                         const size_t topK) {
  auto inds = tensorrt_common::argsort(output.cbegin(), output.cend(), true);
...
inline vector<size_t> topK(const vector<T> inp, const size_t k) {
  ...
  std::vector<size_t> inds =
      tensorrt_common::argsort(inp.cbegin(), inp.cend(), true);
